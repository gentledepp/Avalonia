...
- **Frozen extent (`_frozenExtentU`)**: Lock the extent reported to `ScrollViewer` at the pre-oscillation value. This prevents the scroll anchor mechanism from adjusting the offset in response to oscillating values. The frozen extent converges toward reality via two mechanisms: (1) **Immediate growth**: when the actual calculated extent exceeds the frozen extent by >5px, the frozen value is updated immediately — growing the extent just reveals more scrollable space and never causes viewport jumps; (2) **Stabilized shrink**: when the actual extent is *smaller* and stabilizes within ±5px for 2+ consecutive passes, the frozen value shrinks to match. This asymmetry prevents the "undershoot" problem where a too-small frozen extent traps the user mid-list with no ability to scroll further.
...
- **Bottom boundary**: When the last item is realized (`_hasReachedEnd`) during frozen extent and the frozen extent exceeds the actual content end (`_frozenExtentU > contentEndU`), cap `_frozenExtentU` at `contentEndU`. This prevents the scrollbar from allowing scrolling past the last item. The cap is unconditional — the previous guard (`contentEndU >= viewportUEnd`) was removed because it prevented capping when fast scrolling pushed the viewport past all content, leaving the frozen extent inflated and the ScrollViewer showing an empty screen.
